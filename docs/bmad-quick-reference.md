...
# 3. Implementation
/bmad:bmm:agents:dev
*develop                     # Implement story (make tests pass with Bun Test)
                             # ⚠️  Quality gates run automatically (MANDATORY):
                             # ✅ TypeScript: 0 errors (strict mode, no 'any')
                             # ✅ ESLint: 0 errors (no eslint-disable)
                             # ✅ Tests: 100% pass rate (Bun Test)
                             # ✅ Mutation: 80%+ score (Stryker)
                             # ✅ Formatting: 100% Prettier compliance (Prettier 3.5.3)

# 4. Quality Checks
/bmad:bmm:agents:tea
*test-review                 # Review test quality + mutation testing

/bmad:bmm:agents:dev
*review                      # Code review + quality gates verification
# [User verifies DoD]
...
**Zero-Tolerance Quality Gates** (enforced automatically):

- TypeScript must compile with 0 errors (strict mode)
- ESLint must pass with 0 errors
- Tests must pass 100% (no failures allowed)
- Mutation score must be 80%+ (Stryker threshold)
- Code must be properly formatted (Prettier)

**NEVER use**:

- ❌ `eslint-disable` comments (refactor code instead)
- ❌ `@ts-ignore` / `@ts-expect-error` (fix types instead)
- ❌ Lowering mutation thresholds (add more tests instead)
...
bun run test:mutate      # Mutation testing (Stryker)
bun run quality-gates    # Run all checks
```

### 6. Understanding Quality Gate Enforcement

**How it works**:

1. DEV agent runs all quality checks in **Step 4** of dev-story workflow
2. **Substeps 4.1-4.5** execute in order - **all must pass**
3. If any check fails → Agent **stops and fixes** before continuing
4. Review workflow **verifies** all checks were run (Step 5.5)
...
- CI/CD builds will fail if standards slip
...
bun run format:check     # Prettier (100% compliance)
bun test                 # Unit/integration (100% pass)
bun run test:mutate      # Stryker (80%+ score)
bun run quality-gates    # All checks (if script exists)
```
