...
{{result_story_list}}
  ...
  <substep n="5.5" goal="Verify quality gates enforcement">
    <action>Scan code for eslint-disable comments and flag as violations</action>
    <action>Scan code for @ts-ignore/@ts-expect-error and flag as violations</action>
    <action>Verify TypeScript compilation was successful with 0 errors</action>
    <action>Verify ESLint validation was successful with 0 errors</action>
    <action>Check mutation testing results meet 80%+ threshold if applicable</action>
    <action>Verify code formatting follows Prettier conventions</action>
    <check>If quality violations found → flag as High severity finding</check>
    <check>If eslint-disable or @ts-ignore found → requires changes before approval</check>
    <check>If quality gates not met → story cannot be approved</check>
  </substep>
